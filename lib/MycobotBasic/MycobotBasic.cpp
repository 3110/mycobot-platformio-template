        ...
        case 14:
            ...
            switch (int(r_data_14[0])) {
                ...
                case GET_COORDS: {
                    ...
                    pCoords->at(3) = (temp > 60000 ? (temp - 65536) : temp) /
                                     1000 * COEFFICIENT;
                    temp = ry_low + ry_high * 256;
                    pCoords->at(4) = (temp > 60000 ? (temp - 65536) : temp) /
                                     1000 * COEFFICIENT;
                    temp = rz_low + rz_high * 256;
                    pCoords->at(5) = (temp > 60000 ? (temp - 65536) : temp) /
                                     1000 * COEFFICIENT;
...
Coords MycobotBasic::GetCoords() {
    ...
    while (true) {
        ...
        if (tempPtr == nullptr)
            ...
            for (int i = 0; i < 6; ++i) tempCoords[i] = pCoords->at(i);
            delete pCoords;
            return tempCoords;
        }
    }
...
void MycobotBasic::WriteCoords(Coords coord, int speed) {
    ...
    byte coord_rx_low =
        lowByte(static_cast<int>(coord[3] / COEFFICIENT * 1000));
    byte coord_rx_high =
        highByte(static_cast<int>(coord[3] / COEFFICIENT * 1000));

    byte coord_ry_low =
        lowByte(static_cast<int>(coord[4] / COEFFICIENT * 1000));
    byte coord_ry_high =
        highByte(static_cast<int>(coord[4] / COEFFICIENT * 1000));

    byte coord_rz_low =
        lowByte(static_cast<int>(coord[5] / COEFFICIENT * 1000));
    byte coord_rz_high =
        highByte(static_cast<int>(coord[5] / COEFFICIENT * 1000));
